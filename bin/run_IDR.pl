...
$_ = "" for my($output_dir, $command, $method);
$_ = 0 for my($core, $job_id);
$_ = () for my(@tag_dirs, @input_dirs, @split);
my $idr_path = "/gpfs/data01/glasslab/home/h1bennet/anaconda3/bin/idr";
 
sub printCMD{
        print STDERR "Usage:\n";
        print STDERR "General commands:\n";
        print STDERR "\t-tag_dirs <list of tag directories - comma seperated>\n";
	print STDERR "\t-input_dirs <list of input tag directores - comma seperated >\n";
	print STDERR "\t-output_dir <name of output directory>\n";
	print STDERR "\t-core <number of cores> (default: 4)\n";
	print STDERR "\t-method <chip|atac> (default: chip - requires input)\n";
        exit;
}
...
if($method ne "chip" && $method ne "atac") {
	&printCMD();
}
...
if(@input_dirs == 0 && $method eq "chip") {
	&printCMD();
}

if(@tag_dirs == 0 || $output_dir eq "") {
	&printCMD();
}
...
for(my $i = 0; $i < @tag_dirs; $i++) {
	...
	if($method eq "chip") {
		$command = "findPeaks " . $tag_dirs[$i] . " -L 0 -C 0 -fdr 0.9 -i " . $output_dir . "/pooled_input > " . $output_dir . "/peaks_" . $split[-1] . ".txt 2>> $output_dir/error.txt";
		print STDERR "\t" . $split[-1] . "\n";
		print STDERR "\t" . $command. "\n";
		`$command`;
		$command = "findPeaks " . $output_dir . "/" . $split[-1] . "_pseudo_1 -L 0 -C 0 -fdr 0.9 -i " . $output_dir . "/pooled_input > " . $output_dir . "/peaks_" . $split[-1] . "_pseudo_1.txt 2>> $output_dir/error.txt";
		print STDERR "\t" . $split[-1] . "_pseudo_1\n";
		print STDERR "\t" . $command. "\n";
		`$command`;
		$command = "findPeaks " . $output_dir . "/" . $split[-1] . "_pseudo_2 -L 0 -C 0 -fdr 0.9 -i " . $output_dir . "/pooled_input > " . $output_dir . "/peaks_" . $split[-1] . "_pseudo_2.txt 2>> $output_dir/error.txt";
		print STDERR "\t" . $split[-1] . "_pseudo_2\n";
		print STDERR "\t" . $command. "\n";
		`$command`;
	} else {
		$command = "findPeaks " . $tag_dirs[$i] . " -L 0 -C 0 -fdr 0.9 -minDist 200 -size 200 > " . $output_dir . "/peaks_" . $split[-1] . ".txt 2>> $output_dir/error.txt";
		print STDERR "\t" . $split[-1] . "\n";
		print STDERR "\t" . $command. "\n";
		`$command`;
		$command = "findPeaks " . $output_dir . "/" . $split[-1] . "_pseudo_1 -L 0 -C 0 -fdr 0.9 -minDist 200 -size 200 > " . $output_dir . "/peaks_" . $split[-1] . "_pseudo_1.txt 2>> $output_dir/error.txt";
		print STDERR "\t" . $split[-1] . "_pseudo_1\n";
		print STDERR "\t" . $command. "\n";
		`$command`;
		$command = "findPeaks " . $output_dir . "/" . $split[-1] . "_pseudo_2 -L 0 -C 0 -fdr 0.9 -minDist 200 -size 200 > " . $output_dir . "/peaks_" . $split[-1] . "_pseudo_2.txt 2>> $output_dir/error.txt";
		print STDERR "\t" . $split[-1] . "_pseudo_2\n";
		print STDERR "\t" . $command. "\n";
		`$command`;
	}
...
if($method eq "chip") {
	$command = "findPeaks " . $output_dir . "/pooled_tag_dirs -L 0 -C 0 -fdr 0.9 -i " . $output_dir . "/pooled_input > " . $output_dir . "/peaks_pooled_tag_dirs.txt 2>> $output_dir/error.txt";
	print STDERR "\tpooled_tag_dirs\n";
	print STDERR "\t" . $command. "\n";
	`$command`;

	$command = "findPeaks " . $output_dir . "/pooled_tag_dirs_pseudo_1 -L 0 -C 0 -fdr 0.9 -i " . $output_dir . "/pooled_input > " . $output_dir . "/peaks_pooled_tag_dirs_pseudo_1.txt 2>> $output_dir/error.txt";
	print STDERR "\tpooled_tag_dirs_pseudo_1\n";
	print STDERR "\t" . $command. "\n";
	`$command`;

	$command = "findPeaks " . $output_dir . "/pooled_tag_dirs_pseudo_2 -L 0 -C 0 -fdr 0.9 -i " . $output_dir . "/pooled_input > " . $output_dir . "/peaks_pooled_tag_dirs_pseudo_2.txt 2>> $output_dir/error.txt";
	...
	$command = "findPeaks " . $output_dir . "/pooled_tag_dirs_pseudo_1 -L 0 -C 0 -fdr 0.9 -minDist 200 -size 200 > " . $output_dir . "/peaks_pooled_tag_dirs_pseudo_1.txt 2>> $output_dir/error.txt";
	print STDERR "\tpooled_tag_dirs_pseudo_1\n";
	print STDERR "\t" . $command. "\n";
	`$command`;

	 $command = "findPeaks " . $output_dir . "/pooled_tag_dirs_pseudo_2 -L 0 -C 0 -fdr 0.9 -minDist 200 -size 200 > " . $output_dir . "/peaks_pooled_tag_dirs_pseudo_2.txt 2>> $output_dir/error.txt";
	print STDERR "\tpooled_tag_dirs_pseudo_2\n";
	print STDERR "\t" . $command. "\n";
        `$command`;
}
